...
void write_all_simple_segments_eps(Image I,List_Multiple_Contours contours,FILE *file_simple_eps_1,FILE *file_simple_eps_2){
    Node_Multi_Contour* node;
    node = (Node_Multi_Contour *)malloc(sizeof(Node_Multi_Contour));
    node = contours.head;
    List_Segments L;
    ...
    fprintf(file_simple_eps_2,"%%!PS−Adobe−3.0 EPSF−3.0\n");
	fprintf(file_simple_eps_2,"%%%%BoundingBox: 0 -%d %d 0\n",I.H,I.L);
    while(node!=NULL){
        
        cont = sequence_points_List_vers_tableau(node->data);

        L = simplification_douglas_peucker(cont,0,cont.size-2,1);
        totalNumSimple1 +=L.count;
        write_List_Segments_to_eps(file_simple_eps_1,L);

        L = simplification_douglas_peucker(cont,0,cont.size-2,2);
        totalNumSimple2 += L.count;

        write_List_Segments_to_eps(file_simple_eps_2,L);
        node = node->next_contour;
    }
    fprintf(file_simple_eps_1,"fill\n");
    fprintf(file_simple_eps_1,"showpage\n");
    fclose(file_simple_eps_1);
    fprintf(file_simple_eps_2,"fill\n");
    fprintf(file_simple_eps_2,"showpage\n");
    fclose(file_simple_eps_2);
    printf("Le nombre total de segments des contours de la simplification avec d = 1 : %d\n",totalNumSimple1);
    printf("Le nombre total de segments des contours de la simplification avec d = 2 : %d\n",totalNumSimple2);
}
